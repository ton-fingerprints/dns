...
let LOCALE_CONTROLLER = new LocaleController({
  store,
  localeDict: "index",
}).init();
...
const tonRootAddress = IS_TESTNET
  ? new TonWeb.Address(TON_ROOT_ADDRESS_TESTNET).toString(
      true,
      true,
      true,
      true
...
const tonweb = new TonWeb(
  new TonWeb.HttpProvider(toncenterUrl, {
    apiKey: IS_TESTNET ? TONCENTER_API_KEY_TESTNET : TONCENTER_API_KEY,
  })
);
...
const DEFAULT_CARETE_HELPER_TEXT = ".fingerprints.ton";
  ...
  const loadDomain = async (setShimmers) => {
    ...
    if (currentDomain === domain) {
      if (!domainExists) {
        ...
        const isTakenByUser =
          walletController.getAccountAddress() === currentOwner;
        ...
        if (isTakenByUser) {
          ...
          const expiryDate = new Date(
            lastFillUpTime * 1000 + MS_IN_ONE_LEAP_YEAR
          );
          ...
          const isDomainRenewable =
            isDomainExpired || days <= DOMAIN_RENEW_LIMIT_IN_DAYS;
...
function closeBidModal() {
  ...
  $("#otherPaymentsMethods").removeEventListener(
    "click",
    renderOtherPaymentsMethods
  );
}
...
const onInput = (e) => {
  if (e.key === "Enter") {
    ...
    if (domain.endsWith(".fingerprints.ton")) {
...
const processUrl = () => {
  ...
  const domainFromUrl = decodeURIComponent(
...
const renderAuctionDomain = (domain, domainItemAddress, auctionInfo) => {
  ...
  $("#auctionBidStepConverted").innerText = formatNumber(
    bidStepToPercent.toFixed(2)
  );
  ...
  getCoinPrice().then((price) => {
    if (price) {
      $("#auctionAmountConverted").innerText = formatNumber(
        auctionAmount * price,
        2
      );
    }
...
const renderFreeDomain = async (domain) => {
  ...
  getCoinPrice()
    .then((price) => {
      if (price) {
        $("#freeMinBetConverted").innerText = formatNumber(
          salePrice * price,
          2
        );
      }
...
const renderBusyDomain = (
  ...
  const formattedExpiryDate = expiresDate
    .toISOString()
    .slice(0, 10)
    .split("-")
    .reverse()
...
const renderSearchHistory = (node) => {
  ...
  try {
    const historyMarkup = getHistoryFromStorage().map((historyRecord) => {
  ...
  const onHistoryRecordClick = (e) => {
    ...
    const domain =
      e.target.parentNode.dataset.record || e.target.dataset.record;
...
const attachPaymentModalListeners = (
  ...
  removeListeners[modalButton] = () => {
    $(modalButton).removeEventListener(
      "click",
      togglePaymentModalOnClick,
      false
    );
    showOtherPaymentMethods.removeEventListener(
      "click",
      renderOtherPaymentsMethods
    );
  };
...
function togglePaymentModal({
  ...
  const renderFirstStep = () => {
    ...
    $("#domainName--bid__modal").innerText = domain + ".fingerprints.ton";
  ...
  const handlePaymentConfirmation = async () => {
    ...
    if (!payload) {
      payload =
        modalType === "renew"
          ? await getChangeDnsRecordPayload(message)
          : await getAuctionBidPayload(message);
    }
  ...
  const renderSecondStep = () => {
    ...
    renderQr(
      ...
      "https://app.tonkeeper.com/transfer/" +
        ...
        "&amount=" +
        encodeURIComponent(new BigNumber(localPrice).multipliedBy(1000000000))
    );
    ...
    showOtherPaymentMethods.addEventListener(
      "click",
      renderOtherPaymentsMethods
    );
  };
  ...
  const updateBidModalPaymentData = () => {
    $("#domainName--bid__modal--payment").innerText =
...
function renderOtherPaymentsMethods() {
  ...
  if (otherPaymentsContainer.classList.contains("show")) {
    ...
    otherPaymentsTimerId = setTimeout(() => {
      ...
      otherPaymentsContainer.scrollIntoView({
        behavior: "smooth",
        block: "start",
      });
    }, 100);
...
const renderHistoryContainer = (node) => {
  let historyContainer = node.parentNode.querySelector(
  ...
  try {
    ...
    if (domain === currentDomain) {
      $("#editWalletRow input").value = dnsRecordWallet
        ...
        : "";
      $("#editAdnlRow input").value = dnsRecordSite
        ? dnsRecordSite.toHex()
        : "";
      $("#siteStorage").checked = isSiteInStorage;
      $("#editStorageRow input").value = dnsRecordStorage
        ? dnsRecordStorage.toHex()
        : "";
      ...
      createEditBtn("#editAdnlRow .edit__button").addEventListener(
        "click",
        () => {
      ...
      createEditBtn("#editStorageRow .edit-btn").addEventListener(
        "click",
        () => {
...
$("#manageDomainBtn").addEventListener("click", () =>
  toggleManageDomainForm(currentDomain, currentDnsItem)
);
...
function hideGGElements(domain) {
  ...
  Object.values(ggElements).forEach((node) => {
    if (
      !!node &&
      !node.classList.contains(ggHiddenClassName) &&
...
function renderGGElements(ggDomainData, domain) {
  ...
  if (!!ggDomainData.sale) {
    ...
    if (!!ggDomainData.auction.max_bid) {
      ...
      $("#ggAuctionMaxBidConverted").innerText =
        ggDomainData.auction.max_bid.usd;
...
function setCareeteHelperValue(value) {
  ...
  const cuttedHintText = getSubStrAfterSubStr(
    value,
    DEFAULT_CARETE_HELPER_TEXT[0]
  );

  $(".start-input-container__domain").innerText =
    cuttedHintText && DEFAULT_CARETE_HELPER_TEXT.includes(cuttedHintText)
  ...
  if (value !== oldStartInputValue) {
    ...
    if (careeteHelper) {
      careeteHelper.style.left = `${
        (windowWidth > 568 ? 72 : 56) + width + OFFSET_BETWEEN_TEXT_AND_CARRETE
      }px`;
      ...
      if (
        careeteHelperDimensions.x >
        iconDimensions.x - careeteHelperDimensions.width
      ) {
