...
const getCoinPrice = () => {
  if (
    LAST_PRICE &&
    LAST_PRICE_UPDATED_DATE &&
    Date.now() - LAST_PRICE_UPDATED_DATE < 100 * 60 * 5
  ) {
    // 30 sec
...
const setScreen = (name, ggDomainState) => {
  ...
  toggleClassName(
    "nav .container-inner",
    !["startScreen", "myDomainsView"].includes(name),
    "squizedPadding"
  );
...
const setDomainName = (domain, node) => {
  ...
  const nextDomainName = domain + ".fingerprints.ton";
...
function adjustPaymentModalCaption(modalType) {
  if (modalType === "place a bid") {
    ...
    $("#bid__modal--submit__step--label_1").innerText =
      store.localeDict.place_label;
    $("#bid__modal--submit__step--label_2").innerText =
      store.localeDict.place_label_2;
    $(".bid__modal--payment #domainName--bid__modal--payment").innerText =
      store.localeDict.place_bid;
    $(".bid__modal--second__step #domainName--bid__modal--payment").innerText =
      store.localeDict.place_bid;

    $("#payment-message-success .payment__message--title").innerText =
      store.localeDict.payment_success_header;
    $("#payment-message-success .payment__message--description").innerText =
      store.localeDict.payment_success_description;
  ...
  if (modalType === "manage domain") {
    $("#bidModalSubheader").innerText =
      store.localeDict.manage_domain_payment_explanation;
    $("#bid__modal--bid__input").classList.add("disabled__input");

    $("#bid__modal--submit__step--label_1").innerText = store.localeDict.pay;
    $("#bid__modal--submit__step--label_2").innerText = "";
    $(".bid__modal--payment #domainName--bid__modal--payment").innerText =
      store.localeDict.manage_domain_payment_caption;
    $(".bid__modal--second__step #domainName--bid__modal--payment").innerText =
      store.localeDict.manage_domain_payment_caption;
...
async function getGGDomainData(domainAddressString) {
  try {
    const response = await fetch(
      `${GG_ENDPOINT}/status/${domainAddressString}`
    );
